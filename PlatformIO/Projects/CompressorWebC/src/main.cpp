...
#include <Wire.h>
#include <RTClib.h>       // Inclui a biblioteca RTClib para usar com o RTC
...
void handleRoot();                                     // Declara a função para manipular a página principal
void handleSync();                                     // Declara a função para manipular a sincronização de tempo
...
void setupServer()
{
    ...
    {
        Serial.println("Sistema de arquivos SPIFFS iniciado com sucesso");
    }
    ...
    // Define os endpoints do servidor para sincronização de tempo
