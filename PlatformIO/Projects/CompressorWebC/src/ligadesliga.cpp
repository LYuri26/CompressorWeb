...
const long interval = 10000;      // Intervalo de 10 segundos (em milissegundos) para atualizar o status do compressor
...
bool readCompressorState()
{
    ...
    File file = SPIFFS.open("/estadocompressor.txt", "r");
    if (!file)
    {
...
void saveCompressorState(bool state)
{
    ...
    File file = SPIFFS.open("/estadocompressor.txt", "w");
    if (!file)
    {
...
void setupLigaDesliga(WebServer &server)
{
    ...
    server.on("/toggle", HTTP_GET, [&server]()
        ...
        if (compressorLigado && (currentMillis - lastToggleTime < 10000)) {
            Serial.println("Comando ignorado. O compressor foi alterado recentemente. Aguarde 10 segundos entre as tentativas.");
            server.send(200, "text/plain", "Comando ignorado. Aguarde 10 segundos entre as tentativas.");
            return;
        }
